...
const TCHAR*	g_szWINDOW_CLASS_NAME	= _T("ShardsOfTessara");		//	Window Class Name.
const TCHAR*	g_szWINDOW_TITLE		= _T("Shards of Tessara");		//	Window Title.
...
BOOL RegisterWindowClass(HINSTANCE hInstance)
{
	...
	winClassEx.hIcon			= LoadIcon(NULL, MAKEINTRESOURCE(IDI_APPLICATION));
	winClassEx.hIconSm			= LoadIcon(NULL, MAKEINTRESOURCE(IDI_APPLICATION));
...
LRESULT CALLBACK WindowProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_MOUSEWHEEL:
		{
			short val = GET_WHEEL_DELTA_WPARAM(wParam);
