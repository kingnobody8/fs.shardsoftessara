...
void CSGD_Direct3D::ChangeDisplayParam(int nWidth, int nHeight, bool bWindowed)
{
	...
	if( bWindowed == false )
	{
		...
		for( int i = 0; i < size; i++ )
		{
			...
			//NOTE:	-DANIEL
			//		-4/15 6:07pm.
			//		-I added a cast from 'int' to 'unsigned int' on these two for loops so that the warning did not appear
			if( nWidth == mode.Width && nHeight == mode.Height )
			{
				valid = true;
				break;
			}
			else if( (unsigned int)nWidth <= mode.Width && (unsigned int)nHeight <= mode.Height )
			{
				if( mode.Width <= (unsigned int)nLargerWidth && mode.Height <= (unsigned int)nLargerHeight )
				{
					larger = true;
